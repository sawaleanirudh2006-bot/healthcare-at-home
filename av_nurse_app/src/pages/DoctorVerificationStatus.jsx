...
import { Clock, ShieldAlert, LogOut, FileText, CheckCircle } from 'lucide-react';
...
export default function DoctorVerificationStatus() {
    ...
    useEffect(() => {
        const fetchStatus = async () => {
            ...
            if (session?.user) {
                ...
                // For demo fallback: check local storage if status stored manually or just show pending
                const storedStatus = localStorage.getItem('doctor_verification_status') || 'pending';
    ...
    if (loading) {
        return <div className="min-h-screen flex items-center justify-center bg-slate-50 font-sans">
            <div className="animate-spin w-8 h-8 rounded-full border-4 border-sky-600 border-t-transparent"></div>
        </div>;
    }
            ...
            <div className="max-w-md w-full bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden text-center">
                ...
                {status === 'pending' ? (
                    <div className="p-10 space-y-6">
                        <div className="w-24 h-24 bg-sky-50 rounded-full flex items-center justify-center mx-auto mb-6 outline outline-8 outline-sky-50/50">
                        ...
                        <p className="text-slate-500 font-medium leading-relaxed">
                            Your verification details have been successfully submitted. Our team is reviewing your documents to ensure quality standards.
                        </p>
